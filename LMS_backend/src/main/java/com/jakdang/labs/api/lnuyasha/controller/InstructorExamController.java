...
import com.jakdang.labs.api.lnuyasha.service.ExamSubmissionService;
...
public class InstructorExamController {
    ...
    /**
     * 시험 제출 현황 조회
     * GET /api/instructor/exams/{templateId}/submissions
     */
    @GetMapping("/{templateId}/submissions")
    public ResponseEntity<ResponseDTO<List<AnswerDTO>>> getExamSubmissions(
            @PathVariable String templateId,
            @RequestParam(value = "userId", required = false) String userId) {
        
        try {
            List<AnswerDTO> submissions = examSubmissionService.getExamSubmissions(templateId, userId);
            
            return ResponseEntity.ok(ResponseDTO.createSuccessResponse("시험 제출 현황 조회 성공", submissions));
            ...
            return ResponseEntity.badRequest()
                    .body(ResponseDTO.createErrorResponse(400, "시험 제출 현황 조회 중 오류가 발생했습니다: " + e.getMessage()));
        }
    }
